...
from stillsupport.extensions.views.confirmation import Confirmation
from stillsupport.extensions.views.alert import Alert

if TYPE_CHECKING:
    from stillsupport.extensions.views.queue import Queue
    from stillsupport.stillsupport import StillSupport
...
class Ticket(discord.ui.View):
    def __init__(self, ctx: Context, ticket: Dict, parent: "Queue"):
        ...
        self.add_item(
            discord.ui.Button(
                style=discord.ButtonStyle.link, label="View Ticket", url=url, row=1
            )
        )
    ...
    async def build_embed(self):
        ...
        self.embed.add_field(name="Opened By", value=user, inline=True)
        ...
        self.embed.add_field(
            name="Ticket Members",
            value=humanize_list(added_users) or "None",
            inline=True,
        )

        moderator = "No staff assigned"
        if self.ticket["assigned"]:
            moderator = getattr(
                self.ctx.guild.get_member(self.ticket["assigned"]), "mention", "Removed user"
            )
        self.embed.add_field(name="Assigned To", value=moderator, inline=True)

        timestamp = datetime.fromtimestamp(self.ticket["opened"]).strftime(
            "%B %d, %Y at %H:%M UTC"
        )
        self.embed.add_field(name="Date Opened", value=timestamp, inline=True)

    @discord.ui.button(
        label="Assign Staff", style=discord.ButtonStyle.primary, emoji="\n{SHIELD}",
    )
    async def assign_moderator(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        prompt = discord.Embed(
            title="Assign Staff",
            description=(
                "Mention or provide user ID of the staff member you'd like to assign to this ticket."
                "They must be an Admin or a still.support member (i.e. has the staff role) "
                "and can not be the member who opened this ticket."
            ),
        ...
        if user_response.content.lower() == "cancel":
            ...
            if not moderator:
                ...
                alert = Alert(
                    title="Couldn't assign staff . . .",
                    description="Member ID provided is incorrect.",
                    color=await self.ctx.embed_color(),
                )
        ...
        if not (
            ...
            alert = Alert(
                title="Couldn't assign staff . . .",
                description="Member is not an admin or still.support member. (i.e. they do not have the staff role.)",
            ...
            alert = Alert(
                title="Couldn't assign staff . . .",
                description="You can't assign a member to work their own ticket.",
                color=await ctx.embed_color(),
            )
        ...
        if not alert:
            ...
            alert = Alert(
                ...
                description="A staff member has been successfully assigned, and this ticket is now in progress.",
                color=await ctx.embed_color(),
            )
    ...
    @discord.ui.button(
        label="Add/Remove Member to Ticket",
    ...
    async def ar_member(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        await interaction.response.defer()
        prompt = discord.Embed(
            title="Add/Remove Member to Ticket",
            description=(
                "Mention or provide the member ID of each member you'd like to add. "
                "You can add multiple members by seperating each with a space. Type 'cancel' to cancel."
            ),
            color=await self.ctx.embed_color(),
        )
        ...
        support = await cog.config.guild(ctx.guild).staffroles()
        exit_early = False
                    ...
                    if user.id in self.ticket["added"]:
                        try:
                            ...
                            failed_embed = discord.Embed(
                                ...
                                description=(
                                    "I need `Manage Channel` permissions to add/remove Member(s)."
                                ),
                                color=0xFF0000,
                            )
                            # TODO: uncomment
        ...
        if not exit_early:
            ...
            if failed:
                results_embed.description += humanize_list(
                    ...
                    " couldn't be added/removed. They may be an admin or are already on the ticket.\n"
                )
            # TODO: uncomment
    ...
    @discord.ui.button(
        label="Lock Ticket", style=discord.ButtonStyle.danger, emoji="\n{LOCK}",
    )
    ...
    async def close_ticket(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if guild_settings["archive"]["enabled"]:
            confirm = Confirmation(
                title="Close ticket?",
                description="Are you sure you want to close this ticket? Ticket will be archived.",
                color=await self.ctx.embed_color(),
            )
        else:
            confirm = Confirmation(
                title="Close ticket?",
                description="Are you sure you want to close this ticket? Ticket will be deleted.",
                color=await self.ctx.embed_color(),
            )
    ...
    async def on_timeout(self):
        await self.parent.original.edit(content="Timed out. Restart process and try again.", view=None)