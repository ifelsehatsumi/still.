...
@commands.guild_only()
@commands.group(name="stillsupport")
async def stillsupport(self, ctx: commands.Context):
    """Let members open tickets by reactng to a message."""
    pass


@checks.admin()
@stillsupport.group(invoke_without_command=True, aliases=["set"])
async def settings(self, ctx):
    """Manage settings for StillSupport"""
    await ctx.send_help()
    ...
    await ctx.send(
        ...
        f"[User-closable]:     {guild_settings['memberclose']}\n"
        f"[User-modifiable]:   {guild_settings['memberedit']}\n"
        f"[User-nameable]:     {guild_settings['membername']}\n"
